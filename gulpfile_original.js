...
const awsDeployPath = 'resources/' + pkg.name + '/';
...
gulp.task('_styles', () => {
  ...
  // .pipe(plumber()) //do we need plumber? Maybe if we start seeing gulp errors
  // .pipe(sourcemaps.init()) //maybe use sourcemaps in the future to debug with source files in the browser
    .pipe(sass.sync({
